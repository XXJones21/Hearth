...
producing sound. A persona authors roughly a dozen normalised numbers; one
expression library of relative deltas gives every persona -- including one
created five minutes ago -- the same full range of behaviour.

The behaviour core (the **FaceDirector**) is renderer-free: state
...
(geometry schema, pose channels, the harness cue protocol) lives in
[`wiki/raw/persona-face-spec.md`](../raw/persona-face-spec.md); this page
is the catalogue of what the face actually DOES.

## What a persona brings

Eleven geometry numbers (head proportions, capsule eye size/spacing/
length/lean, mouth proportions), normalised to the head's own bounding
box, plus the `state_colors` block the orb glow already used -- the face's
ink washes toward the active state's colour. Archetypes (`warm_round`,
...
the attack, hold, lerp OUT over the decay (smoothstepped) -- layered on top
of whatever state is playing, so reactions chain naturally. The harness
resolves the voice's non-verbal tags (`[laughter]`, `[sigh]`, ...) to these
names on `tts_chunk_start`; clients never parse text.
...
First user: **listening watches the composer** -- while the input is up,
