...
pub async fn run_mentat_turn(
    ...
    let mut child = Command::new(&config.python_bin)
        ...
        .env("VALINOR_LLAMA_BASE_URL", &config.llama_base_url)
        .env("VALINOR_MENTAT_WORKSPACE_ROOT", &config.workspace_root)
        .env("VALINOR_MENTAT_PROJECT_SLUG", &config.project_slug)
        .env("VALINOR_MENTAT_MODE", "agent")
        .env("VALINOR_MENTAT_HARNESS", "hermes")
        .env("VALINOR_STACK_SERVER", "rust")
